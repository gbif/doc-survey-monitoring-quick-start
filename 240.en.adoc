...
|term:eco[protocolReferences]
|`See Appendix B of Brunbjerg, A.K., Bruun, H.H., Brøndum, L. et al. A systematic survey of regional multi-taxon biodiversity: evaluating strategies and coverage. BMC Ecol 19, 43 (2019). https://doi.org/10.1186/s12898-019-0260-x`+
...
eco:hasMaterialSamples = ‘FALSE’ at all appropriate sampling event levels. 
...
eco:hasVouchers = ‘FALSE’ at all appropriate sampling event levels. 
...
If the quantity reported using these paired terms does not include all organisms of the same taxon sampled/observed in that single occurrence record (e.g., there are 2 or more occurrence records reported for the same event), then eco:isLeastSpecificTargetCategoryQuantityInclusive = ‘FALSE.’ 
